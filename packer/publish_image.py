...
class ArgsNamespace(argparse.Namespace):
    ...
    def resolve(self):
        ...
        if self.license:
            print(globals()["default_license"])
            self.license = self.license.format(**globals())
...
def lookup_image(args):
    if args.source_image is not None:
        proc = sp.run(
            shlex.split(
                f"gcloud compute images describe {args.source_image} "
                f"--project {args.source_project} "
                "--format json"
            ),
            stdout=sp.PIPE,
        ...
        proc = sp.run(
            shlex.split(
                f"gcloud compute images describe-from-family {args.source_image_family} "
                f"--project {args.source_project} "
                "--format json"
            ),
            stdout=sp.PIPE,
...
def main(args: ArgsNamespace):
    ...
    if build.copy:
        ...
        create_cmd = (
            f"gcloud compute images create {image_name} "
            f"--project {build.publish_project} "
            f"--family {image_family} "
            f"--source-image-project {project} "
            f"--source-image {info['name']} "
            f"--licenses {build.license} "
            f'--description "Public Slurm image based on the {build.os_tag} image" '
            "--force"
        )
        print(create_cmd)
        if not confirm():
        ...
        sp.run(shlex.split(create_cmd), check=True)
    else:
...
parser.add_argument("--version", help="slurm-gcp version to publish to")
